...
def advection(particle: Particle, vis):
        ...
        if current_cell and current_cell.wind_vector and current_cell.water_vector and particle.isActive:
            particle.x = (particle.x + int((0.2 * current_cell.wind_vector[0]) + 1.1 * current_cell.water_vector[0])) % 800
            particle.y = (particle.y + int((0.2 * current_cell.wind_vector[1] + 1.1 * current_cell.water_vector[1]))) % 600
