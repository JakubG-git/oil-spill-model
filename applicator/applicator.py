...
def advection(particle: Particle, current_cell: Cell, vis):
    skalar = 10
    ...
    if current_cell and current_cell.wind_vector and current_cell.water_vector and particle.isActive:
        particle.x = (particle.x + (skalar * 0.03 * current_cell.wind_vector[0]) + skalar * 1.1 *
                      current_cell.water_vector[0])
        particle.y = (particle.y + (skalar * 0.03 * current_cell.wind_vector[1]) + skalar * 1.1 *
                      current_cell.water_vector[1])
